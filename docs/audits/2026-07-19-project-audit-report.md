...
> ✅ Исправлено (commit 600688a): глобальный handler в `index.ts` и catch-блоки в `fillRemaining.ts`, `importFromPdf.ts`, `importFromPhoto.ts`, `importFromUrl.ts` — `err.message` больше не попадает клиенту; полная ошибка идёт в `console.error`, клиенту — фиксированное generic-сообщение с 502/500.
...
> ✅ Исправлено (commit 4f38e75, финал 8-шагового рефакторинга по плану `docs/audits/conv-1-decomposition-plan.md`) — `RecipesView.tsx` разбит на оркестратор + 7 новых sibling-файлов: `RecipesEmptyState.tsx` (пустое состояние), `RecipeCard.tsx` (карточка грида: drag-start, selection-mode, allergen-бейдж), `useRecipeFilters.ts` (вся логика поиска/фильтрации/сортировки, вызывается один раз в `RecipesView`), `RecipesToolbar.tsx` + `RecipeFilterSidebar.tsx` (тулбар и десктопный сайдбар, потребляют общий хук пропами — дублирования состояния нет, синхронизация тулбар↔сайдбар перепроверена вручную), `AddRecipeModals.tsx` (5 модалок добавления/редактирования + delete-confirm + cross-tab photo input), `RecipeDetailModal.tsx` (детальная модалка, самое рискованное извлечение). `RecipesView.tsx`: 2617 → 333 строки. Попутно `scaleMacros()` вынесен из инлайн-IIFE в `src/shared/domain/macros.ts` как чистая функция рядом с `sumMacros`/`remainingMacros`, добавлены unit-тесты (`src/shared/domain/__tests__/macros.test.ts`: identity при portionCount=1, ×2, клампинг servings=0→1). Финальный проход (шаг 7) дополнительно нашёл и исправил 2 регрессии, внесённые самой декомпозицией: мёртвый импорт типа `Subfolder` в `RecipesView.tsx` и `react-hooks/set-state-in-effect` в `RecipeDetailModal.tsx` (useEffect, сбрасывающий `portionCount` при смене рецепта, заменён на паттерн «adjust state during render»). Каждый новый файл определяет свой локальный `cn()` (clsx+tailwind-merge) по образцу `ProgramDetailModal.tsx` — новая shared-абстракция не введена. Проверено: `npm run lint` (tsc --noEmit) чисто; `npm run lint:eslint` чисто по всем файлам рефакторинга (общий счётчик долга по проекту снизился с 29 errors/2 warnings до 27 errors/1 warning — за счёт исправления найденных на этом шаге регрессий); `npm run test` — 122/122 (18 файлов); `npm run build` — прод-сборка проходит. Ручной браузерный смоук-тест вкладки «Рецепты» технически не пройден в этой сессии: окружение использует реальный prod Firebase-проект (`rezept-manager-62bd0`) без анонимной/emulator-аутентификации и без сохранённых тестовых учётных данных — `FakeAuthProvider` подключён только в unit-тестах, не в `main.tsx`. Дошли до экрана логина (email/пароль), дальше не пошли осознанно, чтобы не заводить одноразовый аккаунт в продакшн-проекте без согласования.
...
> ✅ Исправлено (commit ad04215) — все 5 вхождений (`ProgramsView.tsx:674`; `ProgramDetailModal.tsx:208,312,1377,1571` — строки сместились из-за Prettier-форматирования в DOC-14) заменены на `crypto.randomUUID()`. Простая замена один-к-одному, логика не менялась. Существующие тесты не проверяли формат/длину `id`, поэтому правок в тестах не потребовалось. Тесты: 119/119 зелёные (18 файлов), `tsc --noEmit` чистый.
...
> ✅ Исправлено (commit 3ece02d) — `deleteAll()` теперь собирает документы через `getDocs()`, затем коммитит их чанками по 500 (лимит Firestore на batch) через `writeBatch(db)` вместо `Promise.all` по одному `deleteDoc`. Каждый чанк атомарен: либо весь чанк применяется, либо ни один документ в нём не удаляется — частичный сбой больше не оставляет корзину в промежуточном состоянии внутри чанка. Теста на этот репозиторий не было и не добавлялось (нет прецедента для Firestore-репозиториев в проекте, полагаемся на typecheck/eslint/build). Проверено: `tsc --noEmit` чисто, `eslint` без новых ошибок (28 problems: 27 errors/1 warning — не изменилось), `npm run test` 122/122, `npm run build` проходит.
...
> ✅ Исправлено (commit a3a8f5b) — корневая причина не в самом `sumMacros()` (лёгкий `.reduce()`), а в том, что `renderDayView`/`renderWeekView`/`renderMonthView`/`renderListView` — обычные функции, вызываемые условно по `plannerViewScale` внутри тела компонента (не отдельные компоненты и не хуки), поэтому `useMemo` нельзя вызывать внутри них (нарушение rules-of-hooks при переключении сценариев). Вместо точечной memo-обёртки на каждую ячейку добавлен `entriesByDate` — `Map<date, PlannerEntry[]>`, вычисляемый через `useMemo` на верхнем уровне компонента с зависимостью только от `plannerEntries`. `getEntriesForDate()` и все inline `plannerEntries.filter(e => e.date === ...)` в Week/Month view заменены на O(1) lookup по карте вместо полного скана истории планера на каждую из дней×приёмы_пищи ячеек при каждом рендере (включая ре-рендеры от несвязанного UI-state вроде открытия add-дропдауна). `selectedDateMacros` вынесен в отдельный `useMemo`. `sumMacros()` оставлен инлайн в `.map()` — после группировки по дате он работает на уже маленьком подмассиве (записи одного дня), а не на всей истории, отдельная мемоизация каждой ячейки избыточна. Один вызов `plannerEntries.filter()` вне рендера (обработчик добавления в корзину, недельный диапазон) не тронут — не рендер-путь, вне скоупа находки. Проверено: `tsc --noEmit` чисто, `eslint` без новых ошибок (28 problems — не изменилось), `npm run test` 122/122 (включая существующий тест на месячный вид `PlannerView.test.tsx`), `npm run build` проходит.
...
> ⚠️ Частично исправлено (commit 70482be), по решению пользователя (2026-07-21) — только `RecipesView.tsx`/`RecipeCard.tsx`: `RecipeCard` обёрнут в `React.memo`, `toggleFavorite`/`handleEdit` — в `useCallback` (иначе `React.memo` бесполезен: новая ссылка на колбэк каждый рендер сводит memo на нет). Особо актуально после CONV-1 — `formData` (форма добавления рецепта) живёт в `RecipesView.tsx`, и раньше каждая буква в поле "Название блюда" ре-рендерила весь грид карточек. `PlannerView.tsx` **не исправлена и отложена** — записи плана рендерятся как inline JSX внутри `.map()` в 4 render-функциях (Day/Week/Month/List), а не отдельным переиспользуемым компонентом; чтобы `React.memo` вообще на что-то влиял, сначала нужно выделить `PlannerEntryCard`-компонент, что по объёму — отдельная мини-декомпозиция, а не точечный perf-фикс. Зафиксировано отдельным пунктом в `ROADMAP.md`. Проверено (для сделанной части): `tsc --noEmit` чисто, `eslint` без новых ошибок (28 problems — не изменилось, в т.ч. нет новых `react-hooks/exhaustive-deps`), `npm run test` 122/122, `npm run build` проходит.
...
> ✅ Исправлено (commit b2835f3) — реализован второй из предложенных вариантов (стриминг с ранним обрывом), а не только проверка заголовка: `Content-Length` проверяется как быстрый путь (если сервер его прислал и он больше лимита — пропускаем без единого чтения тела), но основная гарантия — чтение тела чанками через `imgResp.body.getReader()` с подсчётом накопленного размера; как только он превышает 600 000 байт, чтение прерывается через `reader.cancel()` и кандидат отбрасывается, не дожидаясь конца потока. Это защищает и от серверов, которые не присылают `Content-Length` или присылают неверное значение — заголовку одному доверять нельзя. `worker/` не имеет тестовой инфраструктуры (только `tsc --noEmit`, см. `CLAUDE.md` → Known constraints), поэтому верификация ограничена typecheck. Проверено: `tsc --noEmit` (worker) чисто, `tsc --noEmit` (корень) чисто, `npm run test` 122/122 (не затронуто, но не должно было сломаться).
...
> ✅ Исправлено (commit ac451e7) — новый `helpers/timeout.ts` (`UPSTREAM_TIMEOUT_MS = 25_000`, `isTimeoutError()`). Все 7 вызовов `ai.models.generateContent()` (`calculateKbzhu`, `fillRemaining`, `importFromPhoto`, `importFromPdf` ×2, `importFromUrl`, `generateImageDataUri` — helper используется `generateImage.ts` и как fallback в `importFromUrl.ts`) получили `config.abortSignal: AbortSignal.timeout(...)`. Основные catch-блоки различают таймаут через `isTimeoutError()` и возвращают чистый 504 вместо общего 502, не раскрывая `err.message` (см. Known constraints). `safeFetch()` в `validateExternalUrl.ts` получил свежий `AbortSignal.timeout()` на каждый redirect-хоп — покрывает оба `fetch()` в `importFromUrl.ts` одним изменением; они уже были в мягких catch-блоках (не критичны для успеха импорта), поэтому там таймаут просто ускоряет отказ от конкретного кандидата без отдельной 504-ветки. Побочно найден и исправлен баг: fallback-вызов `generateImageDataUri()` в `importFromUrl.ts` был не обёрнут в try/catch вовсе — с таймаутом он теперь может отклониться раньше, чем раньше, поэтому обёрнут в try/catch с мягким отказом (картинка опциональна, весь импорт не должен из-за неё падать). `worker/` не имеет тестовой инфраструктуры (см. Known constraints), верификация — `tsc --noEmit`. Проверено: `tsc --noEmit` (worker) чисто, `tsc --noEmit` (корень) чисто, `npm run test` 122/122, `npm run build` проходит.
...
> ✅ Исправлено (commit afdf41c) — добавлен guard на 5MB на изображение, до вызова Gemini. Лимит выбран не произвольно, а по уже существующей клиентской подсказке в `AddRecipeModals.tsx` ("JPG, PNG до 5MB"), которая раньше нигде не проверялась — ни на клиенте, ни на сервере, то есть UI обещал лимит, который ничего не обеспечивало. Размер оценивается по длине base64-строки (`3/4 длины минус padding`) без полного decode — дешёвая проверка, не требует держать декодированные байты в памяти. При превышении — 400 с фиксированным сообщением до создания `GoogleGenAI` клиента и любых сетевых вызовов. `worker/` не имеет тестовой инфраструктуры (см. Known constraints), верификация — `tsc --noEmit`. Проверено: `tsc --noEmit` (worker) чисто, `tsc --noEmit` (корень) чисто, `npm run test` 122/122, `npm run build` проходит.
>
> **Раздел 🐢 Производительность закрыт полностью (PERF-1…8), с двумя явными оговорками:** PERF-2 отложена (см. `ROADMAP.md` — требует продуктового решения по пагинации для Cart/Recipes/Programs), PERF-5 закрыта частично (только `RecipesView.tsx`; остаток по `PlannerView.tsx` — отдельный пункт в `ROADMAP.md`, требует сначала выделения `PlannerEntryCard`).
