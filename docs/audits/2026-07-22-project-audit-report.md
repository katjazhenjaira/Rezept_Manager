...
| 12  | Спорное: продуктовые решения            | LOG-4, DEAD-4, DOC-5          | в работе (LOG-4 ✅, DEAD-4 ✅, DOC-5 отложена) |
...
>
> 🔜 **Отложено (2026-07-23) — единственная незакрытая находка отчёта.** Пользователь выбрал **реализацию shared-документов** (не корректировку документации), но проработку механики перенёс на следующую сессию. Контекст, собранный до паузы, чтобы не собирать заново:
>
> - Одними правилами Firestore не обойтись: `ProgramsView` берёт открытую программу из **своего** списка (`DataContext` → `subscribeAll` по `userId`), поэтому чужая программа не отрендерится даже с разрешённым `read`. Нужен отдельный путь загрузки и просмотра «гостевой» программы.
> - Рецепты программы — отдельные документы `recipes/*` с чужим `userId`. Дать к ним доступ можно либо копией внутри shared-документа, либо маркером на каждом рецепте, либо `get()`-лукапом в правилах.
> - Точки в коде: генерация ссылки — `ProgramsView.handleShareProgram()` (`?programId=`) и `RecipeDetailModal` (`?recipeId=`); приём ссылки — два `useEffect` в `App.tsx` (`programId` → `confirm('Добавить программу…')`, `recipeId` → поиск в своём массиве); правила — `firestore.rules:36-44`.
> - `firestore.rules` деплоится **вручную через Firebase Console** (CLI не настроен) — любая правка правил требует шага на стороне пользователя, иначе фича не работает в проде.
>
> Нерешённые вопросы к следующей сессии: (1) механика — самодостаточный снимок в `sharedPrograms/{token}` (простые правила, не обновляется) vs живой доступ по флагу (актуально, но дорогие правила и массовые записи) vs прокси через Cloudflare Worker (правила не трогаем, но нужен service-account и Firebase Admin); (2) что получатель делает со ссылкой — только просмотр или «добавить себе» (импорт копии программы и её рецептов); (3) охват — только программы или ещё и одиночные рецепты (`?recipeId=`, ломается ровно так же).
...
>
> ✅ Исправлено (commit 8036194) — продуктовое решение пользователя: персистить **в профиле** (Firestore), а не в localStorage, т.к. это пользовательская настройка и должна работать между устройствами. `UserProfile.mealTypes?: string[]` (опционально — профили, созданные раньше, поля не имеют), `DEFAULT_MEAL_TYPES` в `shared/domain/defaults.ts`, чтение в `App.tsx` через `userProfile.mealTypes ?? DEFAULT_MEAL_TYPES`, запись — `saveUserProfile({...userProfile, mealTypes})` с catch и алертом. Попутно добавлен конвертер `stringArray()` с проверкой формы (`allergies` переведён на него же вместо голого каста). Тесты: 5 новых в `App.test.tsx` (мок `PlannerView` отдаёт `mealTypes` и кнопку добавления) + 5 в `converters.test.ts`.
>
> ⚠️ Остаточное поведение (осознанно вне scope): записи планера, созданные **до** этого фикса с уже потерянным пользовательским типом, по-прежнему не отображаются, пока такой тип не будет добавлен заново — вариант «рендерить прочие типы отдельной группой» пользователем не выбран.
...
>
> ✅ Исправлено (commit 0a3b3bf) — продуктовое решение пользователя: **убрать проп**. Продукт не обещает чистку рецептов при удалении категории, а массовая запись по всей библиотеке ради необещанного поведения не оправдана. Удалены поле в `Props`, деструктуризация, вызов в `confirmRemoveCategory()` и заглушка `onCategoryRemoved={() => {}}` в `App.tsx`. Тест «удаляет только кастомную категорию после подтверждения» сохранён и проверяет то же поведение без пропа (фильтрация `availableCategories`).
