...
- [ ] Проработать отчёт аудита кода `docs/audits/2026-07-19-project-audit-report.md` (55/60 закрыто, 1 частично; CONV-1 — декомпозиция `RecipesView.tsx` завершена по плану `docs/audits/conv-1-decomposition-plan.md`; PERF-1, PERF-3, PERF-4 исправлены; PERF-5 частично (только RecipesView.tsx); PERF-2 отложена, см. отдельный пункт ниже; осталось: PERF-6…8)
