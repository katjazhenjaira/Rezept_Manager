...
- [ ] TODO: обновить прод-секрет `GEMINI_API_KEY` на задеплоенном Worker (`wrangler secret put GEMINI_API_KEY`, из `worker/`) — ключ уже перевыпущен под аккаунтом пользователя и обновлён локально (`worker/.dev.vars`), но прод пока не тронут (live-трафика ещё нет). Требует решения вопроса с доступом к Cloudflare-аккаунту — Worker пока задеплоен на личном аккаунте партнёра, `wrangler` не залогинен на этой машине (см. память о переносе Cloudflare-аккаунта).

## Баги

- [x] Импорт рецепта по ссылке не работает (вкладка «Рецепты» → «Импорт по ссылке»). **Диагностировано и исправлено (2026-07-22):** это не был дефект кода — цепочка `AddRecipeModals.handleLinkSubmit` → `aiClient.importFromUrl()` → Worker `/api/ai/import-from-url` → Gemini работала корректно. Причина — невалидный/отозванный `GEMINI_API_KEY` (создан партнёром пользователя, доступ к аккаунту создателя под вопросом). Подтверждено воспроизведением идентичной ошибки `API_KEY_INVALID` на несвязанном AI-роуте (`/api/ai/calculate-kbzhu`) с тем же ключом — значит, проблема не специфична для `importFromUrl`. Ключ перевыпущен пользователем в Google AI Studio под своим аккаунтом (подписка Google AI Pro/Ultra), обновлён в `worker/.dev.vars`. Локально проверено: оба роута возвращают `200 OK`. Прод-секрет (`wrangler secret put GEMINI_API_KEY`) пока не обновлён — см. TODO ниже в «Технический долг».
