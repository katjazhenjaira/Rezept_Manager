...
- [ ] Проработать отчёт аудита кода `docs/audits/2026-07-22-project-audit-report.md` (39 находок: 2 критических, 6 противоречий с документацией, 3 недокументированных, 10 логических, 6 dead code, 4 TypeScript, 2 конвенции, 1 perf, 5 по тестовому покрытию). Разбито на 13 групп по файловой локальности — план и порядок в `docs/plans/2026-07-22-audit-followup-groups.md`, текущий статус — таблица «Прогресс проработки» в начале самого отчёта. **Состояние на 2026-07-23: закрыто 38 из 39** — группы 1-11b `✅` (обе safety-critical находки устранены, фронтенд-тесты 177 → 293), группа 12 закрыта на 2/3: LOG-4 (`mealTypes` персистятся в профиле, commit `8036194`) и DEAD-4 (мёртвый проп `onCategoryRemoved` удалён, commit `0a3b3bf`). Осталась **DOC-5** — реальный межпользовательский шеринг программ: пользователь выбрал реализацию shared-документов, но проработку механики перенёс на следующую сессию; собранный контекст и три нерешённых вопроса записаны прямо в блоке DOC-5 отчёта
