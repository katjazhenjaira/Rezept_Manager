...
import { AddRecipeModals, type AddRecipeModalsProps, type RecipeFormData } from '../AddRecipeModals';
...
const blankFormData: RecipeFormData = {
  title: '',
  author: '',
  sourceUrl: '',
  image: null,
...
describe('AddRecipeModals — handleAddManual', () => {
  ...
  function renderModal(overrides: Partial<AddRecipeModalsProps> = {}) {
    const defaultProps: AddRecipeModalsProps = {
      recipes: [],
      programs: [],
      availableCategories: [],
      recipeTarget: null,
  ...
  it('сбрасывает formData при отмене редактирования (кнопка «Отмена»)', () => {
    const onFormDataChange = vi.fn();
    const onEditingIdChange = vi.fn();
    renderModal({
      editingId: 'existing-recipe-id',
      formData: manualFormData,
      onFormDataChange,
      onEditingIdChange,
    });
    ...
    expect(onEditingIdChange).toHaveBeenCalledWith(null);
    expect(onFormDataChange).toHaveBeenCalledWith(blankFormData);
  });
});
