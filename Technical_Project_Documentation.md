...
- `src/features/tracker/` — TrackerView smoke tests; ProgramSelectionModal: сброс на «По умолчанию», выбор программы и подпапки с цепочкой наследования КБЖУ (подпапка → программа → профиль → 0), создание своего плана
- `src/features/programs/` — ProgramDetailModal: drag-n-drop перенос рецептов (корень ↔ подпапки, между подпапками, no-op при совпадении источника и цели), редактирование целей и ограничений программы/подпапки (успех и ошибка записи), добавление продуктов программы в корзину (разбор «название + количество», пометка базовых, ошибка записи)
...
- `ProgramDetailModal.handleSubfolderPdfUpload`, ветка `activeResourceForm` (запись ресурса в программу/подпапку) — недостижима из UI: `setActiveResourceForm` нигде не вызывается с непустым значением, поэтому и модалка «Добавить документ/ссылку», и эта ветка обработчика — мёртвый код. Юнит-тест написать нельзя (состояние не поднять через UI, обработчик не экспортируется); покрыта только достижимая ветка `editingEntity`. Корректность ветки (await + try/catch, LOG-5) обеспечена ревью; удаление мёртвого кода — отдельная находка, см. заметку к LOG-5 в `docs/audits/2026-07-22-project-audit-report.md`.
