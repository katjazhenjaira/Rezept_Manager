...
Разработка ведётся по принципу **TDD (test-driven development)**. Цель — чтобы всё приложение (`src/` и `worker/src/`) оставалось покрытым тестами. Правило применяется к **любому** изменению кода — новая фича, багфикс, рефакторинг.

**Цикл на каждое изменение:**

1. **Сначала тест, потом код.** Перед написанием/правкой реализации:
   - для новой логики — написать падающий тест, описывающий ожидаемое поведение (**red**), затем реализацию до зелёного (**green**), затем рефактор;
   - для багфикса — сначала тест, воспроизводящий баг (падает), затем фикс (тест зеленеет) — так баг не вернётся;
   - для изменения существующего поведения — сначала обновить/добавить тесты под новое поведение, увидеть их падение на старом коде, потом менять код.
2. **Проверить существующие тесты.** Перед изменением модуля — найти его co-located тесты (`**/__tests__/*.test.ts(x)`), прогнать, понять что они гарантируют. После изменения — все затронутые тесты должны оставаться зелёными (либо осознанно обновлены под новое поведение).
3. **Не оставлять новый экспортируемый код без тестов.** Любая новая экспортируемая функция/хук/роут/middleware в `src/` или `worker/src/` должна получить тест в том же цикле. Safety-critical логика (см. «Safety-critical constraints») — тесты обязательны, приоритет наивысший.
4. **Прогнать перед коммитом:** `npm test` (фронтенд), `npm --prefix worker test` (воркер), `npm run lint` (tsc), при необходимости `npm run test:coverage`. Коммит — только на зелёном.

**Если тест невозможен** (или требует ненастроенной инфраструктуры — эмулятора, Canvas, реального upstream, workerd, E2E-раннера):

- **НЕ** оставлять код молча непокрытым и **НЕ** писать псевдотест, который ничего не проверяет.
- Задокументировать сценарий в `Technical_Project_Documentation.md` → раздел «9. Тестирование» → подраздел **«Нетестируемые сценарии»**, обязательно **с причиной** (почему юнит-тест невозможен и чем поведение проверяется вместо этого — ручная проверка, contract-тест на Fake, E2E-TODO).
- Если непокрытость временная (нужна инфраструктура) — добавить пункт в «Технический долг» `ROADMAP.md`.

**Инфраструктура:** vitest в двух пакетах (корневой + `worker/`), `@testing-library/react` для UI, node env + фейки (KV, `@google/genai`) для воркера. Детали, актуальные числа тестов и полный список нетестируемых сценариев — в `Technical_Project_Documentation.md` §9. Категория `TEST-` скилла `project-audit` проверяет соблюдение этого правила при аудите.
