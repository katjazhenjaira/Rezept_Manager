...
- **`npm run format` не запускать — форматировать точечно.** Выявлено (2026-07-22): в репозитории ~20 файлов, не приведённых к prettier, поэтому `npm run format` (`prettier --write .`) переформатирует посторонние файлы и загрязняет дифф текущей задачи. Правильно: `npx prettier --write <изменённый файл>`.
- **В тестах «сегодня» вычислять локальной датой, а не `toISOString()`.** Выявлено (2026-07-23): компоненты определяют текущий день через `format(new Date(), 'yyyy-MM-dd')` из date-fns (локальная зона), а `new Date().toISOString().slice(0, 10)` даёт **UTC**-дату — в окне между локальной полуночью и полуночью UTC фикстуры планера перестают совпадать с фильтром компонента, и тест падает по времени суток. Образец — `PlannerView.test.tsx` / `TrackerView.test.tsx`.
- **UI-состояние после асинхронной операции проверять через `waitFor`, а не синхронно.** Выявлено (2026-07-23, `AppHeader.test.tsx`): смена языка в i18next резолвится отдельным промисом, и синхронный `expect` сразу после `waitFor` на `i18n.language` ловит ещё не применённый ререндер — тест падает примерно раз в три полных прогона, оставаясь зелёным при изолированном запуске файла. Поэтому финальную верификацию сессии делать несколькими полными прогонами `npm test` подряд: одиночный прогон флейк не показывает.
...
3. **Отмечать выполненные пункты прямо в файле отчёта** (`docs/audits/YYYY-MM-DD-project-audit-report.md`) сразу после коммита — например, дописывать в конец находки `✅ Исправлено (commit <hash>)` или менять её статус. Отчёт должен всегда отражать актуальное состояние проработки. Отметку оформлять **отдельным маленьким коммитом** `docs(audit): отметить <ID> как исправленный`, а не через `git commit --amend`: amend меняет hash, и записанный в отчёт хеш становится висячим (проверено 2026-07-22 на CRIT-1).
